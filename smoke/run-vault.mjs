...
const dom = new JSDOM('<!doctype html><html><head><title>Vault</title></head><body><div id="root"></div></body></html>', { url: 'https://sentientdash.app/vault.html', pretendToBeVisual: true });
for (const key of ['window', 'document', 'navigator', 'HTMLElement', 'Element', 'Node', 'Event', 'KeyboardEvent', 'MouseEvent', 'requestAnimationFrame', 'cancelAnimationFrame', 'getComputedStyle', 'localStorage', 'URLSearchParams']) {
...
globalThis.__VAULT_TEST_ROLE = process.argv[2] || 'dev';

const build = await esbuild.build({
  entryPoints: ['smoke/vault-entry.jsx'], bundle: true, write: false, format: 'esm', platform: 'browser', jsx: 'automatic', target: 'es2022',
