...
try {
  ...
  for (const card of el.querySelectorAll('.test-card')) await act(async () => card.click());
  if (document.querySelectorAll('.stack-card-shell.is-multiselected').length !== 2) throw new Error('Grouping mode must select with a normal click');
  await act(async () => [...document.querySelectorAll('.stack-selection-bar button')].find((node) => /Group selected|Agrupar seleccionados/.test(node.textContent)).click());
