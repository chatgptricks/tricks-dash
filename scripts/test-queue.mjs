...
import { finishesBeforeDeadline, intervalsConflict, planQueueDrop } from '../src/queuePlanner.js';
...
const base = { post: { account: 'chatgptricks' }, durationMinutes: 30, deadlineAt: '2026-09-01T18:00:00Z' };
assert.equal(finishesBeforeDeadline(base, '2026-09-01', 690), true);
assert.equal(finishesBeforeDeadline(base, '2026-09-01', 700), false);
...
const impossible = planQueueDrop({ tasks: [], target: { ...target, deadlineAt: '2026-09-01T14:10:00Z' }, designerEmail: 'pd@example.com', scheduledDate: '2026-09-01', desiredStart: 500, notBefore: 500 });
assert.equal(impossible.ok, false, 'work that cannot finish by its deadline must be rejected');
