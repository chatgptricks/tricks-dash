...
function DashboardView({ viewer }) {
  ...
  return <div className="m-stack">
    ...
    <div className="m-post-grid">{filtered.slice(0, 300).map((post) => <button className="m-post-card" key={`${post.account}:${post.shortcode}`} data-context-type="post" data-context-title={post.title || post.caption || post.account} data-context-account={post.account || ''} data-context-shortcode={post.shortcode || ''} data-context-permalink={post.permalink || ''} onClick={() => setSelected(post)}><div className="m-post-cover"><Cover src={post.coverUrl} alt="" />{post.isHot ? <span className="m-hot"><Flame size={11} />{Number(post.hotMultiplier || 0).toFixed(1)}x</span> : null}{post.queueState ? <span className="m-queued"><Check size={11} /></span> : null}</div><div><b>@{post.account}</b><span><Heart size={11} />{fmt(post.likes)}</span></div></button>)}</div>
