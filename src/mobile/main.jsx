...
import { rankTopicPosts, editorialStates } from '../topicGroups';
...
const I18N = {
  en: {
    ...
    assign: 'Assign', editAssignment: 'Edit assignment', duplicateRequest: 'Duplicate request', returnPool: 'Return to pool', designer: 'Designer', date: 'Date', time: 'Time',
    ...
    title: 'Title', postType: 'Post type', notes: 'Notes', create: 'Create', created: 'Post created in the pool.', sourceLink: 'Source link', getSourceDetails: 'Get details', gettingSourceDetails: 'Getting details…', sourcePreview: 'Source preview',
  ...
  es: {
    ...
    assign: 'Asignar', editAssignment: 'Editar asignación', duplicateRequest: 'Duplicar request', returnPool: 'Devolver al pool', designer: 'Designer', date: 'Fecha', time: 'Hora',
    ...
    title: 'Título', postType: 'Tipo de post', notes: 'Notas', create: 'Crear', created: 'Post creado en el pool.', sourceLink: 'Link de origen', getSourceDetails: 'Traer detalles', gettingSourceDetails: 'Obteniendo detalles…', sourcePreview: 'Vista previa del origen',
...
function HomeView({ viewer, navigate }) {
  ...
  const load = useCallback(() => { setError(''); Promise.all([apiJson(`/api/dashboard/queue/v2?date=${DAY()}`), apiJson('/api/tracker/summary').catch(() => null)]).then(([queue, track]) => { setData(queue); setTracker(track); }).catch((err) => setError(err.message)); }, []);
  useEffect(() => { load(); }, [load]);
...
function DashboardView({ viewer }) {
  const [topicView, setTopicView] = useState(false); const [championMetric, setChampionMetric] = useState('likes'); const [separateTopics, setSeparateTopics] = useState([]); const [count, setCount] = useState(60);
  const { t } = usePrefs();
  ...
  const { groups: topics, loading: grouping } = useTopicGroups(filtered, topicView, separateTopics);
  ...
  return <div className="m-stack">
    ...
    <div className="m-chip-scroll"><button className={!topicView ? 'is-on' : ''} onClick={() => setTopicView(false)}>Posts</button><button className={topicView ? 'is-on' : ''} onClick={() => setTopicView(true)}>Topics</button>{topicView ? <label>Champion by <select value={championMetric} onChange={(event) => setChampionMetric(event.target.value)}><option value="likes">Likes</option><option value="comments">Comments</option><option value="interactions">Likes + comments</option></select></label> : null}</div>
    {grouping ? <p role="status">Grouping similar posts…</p> : null}
    <div className="m-post-grid">{(topicView ? topics : filtered.map((post) => ({ id: `${post.account}:${post.shortcode}`, posts: [post] }))).slice(0, count).map((group) => { const ranked = rankTopicPosts(group.posts, championMetric); const post = ranked[0]; return <div className="m-topic-group" key={group.id}>{topicView && ranked.length > 1 ? <small>Champion · {ranked.length} versions</small> : null}<button className="m-post-card" key={`${post.account}:${post.shortcode}`} data-context-type="post" data-context-title={post.title || post.caption || post.account} data-context-account={post.account || ''} data-context-shortcode={post.shortcode || ''} data-context-permalink={post.permalink || ''} onClick={() => setSelected(post)}><div className="m-post-cover"><Cover src={post.coverUrl} alt="" />{post.isHot ? <span className="m-hot"><Flame size={11} />{Number(post.hotMultiplier || 0).toFixed(1)}x</span> : null}{post.queueState ? <span className="m-queued">{editorialStates[post.queueState] || post.queueState}</span> : null}</div><div><b>@{post.account}</b><span><Heart size={11} />{fmt(post.likes)}</span></div></button>{topicView && ranked.length > 1 ? <details><summary>Compare versions</summary>{ranked.slice(1).map((variant) => <div key={variant.postKey || `${variant.account}:${variant.shortcode}`}><button onClick={() => setSelected(variant)}>@{variant.account} · {fmt(variant.likes)} likes</button><button onClick={() => setSeparateTopics((keys) => [...keys, variant.postKey || `${variant.account}:${variant.shortcode}`])}>Separate</button></div>)}</details> : null}</div>; })}</div>
    {count < (topicView ? topics.length : filtered.length) ? <button className="m-secondary" onClick={() => setCount((value) => value + 60)}>Load more</button> : null}
...
function PickSheet({ items, onClose, onPicked }) {
  ...
  const pick = async () => { setBusy(true); try { const now = new Date(); const placement = { scheduled_date: DAY(now), scheduled_start_minutes: String(Math.min(1430, Math.ceil((now.getHours() * 60 + now.getMinutes()) / 10) * 10)) }; const body = task.isHotCandidate ? new URLSearchParams({ hot_account: task.post.account, hot_shortcode: task.post.shortcode, ...placement }) : new URLSearchParams({ request_id: String(task.id), ...placement }); await apiJson('/api/dashboard/queue/v2/pick', { method: 'POST', body }); onPicked(); onClose(); } catch (error) { setNotice(error.message); setBusy(false); } };
...
function AccountEditSheet({ account, onClose, onSaved }) {
  const { t } = usePrefs(); const [form, setForm] = useState({ label: account.label || '', group: account.group || 'sentient', threshold: account.hot_threshold || 600, scrapeMode: account.scrape_mode || 'posts' }); const [busy, setBusy] = useState(''); const [notice, setNotice] = useState('');
  const save = async (event) => { event.preventDefault(); setBusy('save'); setNotice(''); try { await apiJson(`/api/admin/accounts/${encodeURIComponent(account.handle)}/settings`, { method: 'POST', body: new URLSearchParams({ password: LEGACY_PASSWORD, label: form.label, group: form.group, hot_threshold: String(form.threshold), scrape_mode: form.scrapeMode }) }); setNotice(t('accountSaved')); onSaved(); } catch (error) { setNotice(error.message); } finally { setBusy(''); } };
