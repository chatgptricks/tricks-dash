...
import {
  ...
  MoreHorizontal, Palette, Play, Plus, RefreshCw, Search, Send, Settings, Shield, Sparkles,
  Sun, TimerReset, UserRound, Users, Wifi, WifiOff, X,
} from 'lucide-react';
...
const I18N = {
  en: {
    ...
    newest: 'Newest', mostLiked: 'Most liked', account: 'Account', format: 'Format', clear: 'Clear', results: 'results',
    ...
    followers: 'Followers', growth7d: '7-day growth', avgLikes: 'Average likes', trackedAccounts: 'Tracked accounts',
    ...
    active: 'Active', inactive: 'Inactive', threshold: 'HOT threshold', disk: 'Disk', slack: 'Slack', ocr: 'OCR',
    ...
    language: 'Language', theme: 'Theme', accent: 'Accent', dark: 'Dark', light: 'Light', custom: 'Custom', close: 'Close',
  ...
  es: {
    ...
    newest: 'Más recientes', mostLiked: 'Más likes', account: 'Cuenta', format: 'Formato', clear: 'Limpiar', results: 'resultados',
    ...
    followers: 'Seguidores', growth7d: 'Crecimiento 7 días', avgLikes: 'Promedio de likes', trackedAccounts: 'Cuentas monitoreadas',
    ...
    active: 'Activa', inactive: 'Inactiva', threshold: 'Umbral HOT', disk: 'Disco', slack: 'Slack', ocr: 'OCR',
    ...
    language: 'Idioma', theme: 'Tema', accent: 'Acento', dark: 'Oscuro', light: 'Claro', custom: 'Personalizado', close: 'Cerrar',
...
function PrefsProvider({ children }) {
  ...
  const [accent, setAccentState] = useState(() => localStorage.getItem('sentient.accent') || 'lime');
  const [customAccent, setCustomAccentState] = useState(() => localStorage.getItem('sentient.accentCustom') || '#00ac80');
  ...
  const setCustomAccent = (value) => { localStorage.setItem('sentient.accentCustom', value); document.documentElement.style.setProperty('--custom-accent', value); setCustomAccentState(value); setAccent('custom'); };
  useEffect(() => { document.documentElement.dataset.theme = theme; document.documentElement.dataset.accent = accent; document.documentElement.style.setProperty('--custom-accent', customAccent); }, [theme, accent, customAccent]);
...
function Cover({ src, alt = '', className = '' }) {
  ...
  return failed || !src ? <div className={`m-cover-empty ${className}`}><Sparkles size={22} /></div> : <img className={className} src={assetUrl(src)} alt={alt} loading="lazy" onError={() => setFailed(true)} />;
}
...
function ProfileSheet({ user, viewer, installPrompt, onInstall, onSettings, onSignOut, onClose }) {
  ...
  return <Sheet title={displayName(user.email)} onClose={onClose}>
    ...
    <section className="m-pref-section"><label>{t('accent')}</label><div className="m-colors">{['lime', 'blue', 'coral'].map((color) => <button key={color} className={`is-${color}${prefs.accent === color ? ' is-on' : ''}`} onClick={() => prefs.setAccent(color)} aria-label={color} />)}<label className={prefs.accent === 'custom' ? 'is-on' : ''}><Palette size={14} /><input type="color" value={prefs.customAccent} onChange={(event) => prefs.setCustomAccent(event.target.value)} /></label></div></section>
...
function HomeView({ viewer, navigate }) {
  ...
  return <div className="m-stack">
    ...
    <div className="m-section-head"><div><span>{t('tracker')}</span><h2>{t('trackedAccounts')}</h2></div><button onClick={() => navigate('tracker')}><ArrowUpRight size={15} /></button></div>
    <div className="m-mini-list">{(tracker?.accounts || []).slice(0, 3).map((account) => <div key={account.handle}><Cover src={`/api/dashboard/avatar/${account.handle}`} /><span><b>@{account.handle}</b><small>{fmt(account.followers)} {t('followers')}</small></span><strong className={(account.delta_7d?.delta || 0) < 0 ? 'is-negative' : ''}>{account.delta_7d ? `${account.delta_7d.delta > 0 ? '+' : ''}${fmt(account.delta_7d.delta)}` : '—'}</strong></div>)}</div>
  </div>;
}
...
function DashboardView({ viewer }) {
  ...
  const [search, setSearch] = useState(''); const [group, setGroup] = useState('all'); const [account, setAccount] = useState(''); const [type, setType] = useState(''); const [sort, setSort] = useState('newest');
  ...
  const filtered = useMemo(() => {
    ...
    const rows = (payload?.posts || []).filter((post) => !post.hidden).filter((post) => {
      ...
      if (query && !`${post.caption || ''} ${post.ocrText || ''} ${post.account || ''} ${post.musicSong || ''}`.toLowerCase().includes(query)) return false;
      return true;
    });
    rows.sort(sort === 'likes' ? (a, b) => (b.likes || 0) - (a.likes || 0) : (a, b) => String(b.postDate || '').localeCompare(String(a.postDate || '')));
    return rows;
  }, [payload, search, group, account, type, sort]);
  if (!payload && !error) return <Spinner label={t('loading')} />;
  return <div className="m-stack">
    <div className="m-search-row"><label><Search size={17} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder={t('search')} /></label><button className={account || type || sort !== 'newest' ? 'is-active' : ''} onClick={() => setFiltersOpen(true)}><Filter size={18} /></button></div>
    ...
    <div className="m-result-count"><span>{fmt(filtered.length)} {t('results')}</span>{(account || type || search) ? <button onClick={() => { setAccount(''); setType(''); setSearch(''); }}>{t('clear')}</button> : null}</div>
...
function QueueView({ viewer }) {
  ...
  return <div className="m-stack">
    ...
    {mode === 'agenda' ? <section className="m-agenda"><div className="m-section-head"><div><span>{dateLabel(date)}</span><h2>{coordinator ? t('team') : t('myDay')}</h2></div></div>{(coordinator ? team.filter((task) => task.scheduledDate === date) : assigned.filter((task) => task.scheduledDate === date)).length ? (coordinator ? team : assigned).filter((task) => task.scheduledDate === date).map((task) => <QueueTaskCard key={task.id} task={task} onClick={() => setOpen(task)} showDesigner={coordinator} />) : <Empty title={t('noAssignments')} />}</section> : null}
...
function TrackerView() {
  const { t, language } = usePrefs(); const [data, setData] = useState(null); const [detail, setDetail] = useState(null); const [error, setError] = useState(''); const [busy, setBusy] = useState('');
  ...
  return <div className="m-stack"><div className="m-metric-grid"><Metric label={t('trackedAccounts')} value={accounts.length} /><Metric label={t('followers')} value={fmt(accounts.reduce((sum, account) => sum + Number(account.followers || 0), 0))} /><Metric label={t('trackingSince')} value={data?.tracking_since ? dateLabel(data.tracking_since, language) : '—'} /></div><Notice type="error">{error}</Notice><section className="m-account-list">{accounts.map((account) => <button key={account.handle} onClick={() => open(account.handle)}><Cover src={`/api/dashboard/avatar/${account.handle}`} /><div><b>@{account.handle}</b><span>{account.label}</span></div><strong>{fmt(account.followers)}<small className={(account.delta_7d?.delta || 0) < 0 ? 'is-negative' : ''}>{account.delta_7d ? `${account.delta_7d.delta > 0 ? '+' : ''}${fmt(account.delta_7d.delta)} · 7d` : '— · 7d'}</small></strong>{busy === account.handle ? <LoaderCircle className="spin" size={16} /> : <ChevronRight size={18} />}</button>)}</section>{detail ? <TrackerDetail data={detail} busy={busy === detail.handle} onRefresh={() => refresh(detail.handle)} onClose={() => setDetail(null)} /> : null}</div>;
}
...
function TrackerDetail({ data, busy, onRefresh, onClose }) {
  ...
  return <Sheet title={`@${data.handle}`} onClose={onClose} wide><div className="m-stack"><div className="m-detail-account"><Cover src={`/api/dashboard/avatar/${data.handle}`} /><div><h3>{data.label || data.handle}</h3><span>{fmt(points.at(-1))} {t('followers')}</span></div><button onClick={onRefresh} disabled={busy}><RefreshCw className={busy ? 'spin' : ''} size={17} /></button></div><div className="m-sparkline"><svg viewBox="0 0 300 100" role="img"><path d={path} /></svg></div><div className="m-section-head"><div><span>{t('history')}</span><h2>{history.length} snapshots</h2></div></div><div className="m-history-list">{history.slice(-12).reverse().map((row) => <div key={row.date}><span>{dateLabel(row.date)}</span><b>{fmt(row.followers)}</b><small>{row.posts_that_day || 0} {t('posts')}</small></div>)}</div></div></Sheet>;
}
...
function SettingsView({ viewer }) {
  const { t } = usePrefs(); const allowed = viewer.is_admin || viewer.is_dev; const [tab, setTab] = useState('overview'); const [data, setData] = useState({}); const [loading, setLoading] = useState(true); const [error, setError] = useState(''); const [editingUser, setEditingUser] = useState(null);
  ...
  return <div className="m-stack"><div className="m-tab-scroll">{tabs.map((value) => <button key={value} className={tab === value ? 'is-on' : ''} onClick={() => setTab(value)}>{t(value)}</button>)}</div>{loading ? <Spinner label={t('loading')} /> : null}<Notice type="error">{error}</Notice>{!loading && tab === 'overview' ? <div className="m-command-grid"><Metric label={t('accounts')} value={data.accounts.length} /><Metric label={t('users')} value={data.users.length} /><Metric label={t('usage')} value={data.usage?.active_users_30d ?? '—'} hint="30d" /><Metric label={t('disk')} value={`${data.disk?.pct_used ?? '—'}%`} /><Metric label={t('approvals')} value={data.report?.totals?.scheduled?.count ?? 0} /><Metric label="OCR" value={data.ocr?.remaining ?? '—'} /></div> : null}{!loading && tab === 'accounts' ? <section className="m-settings-list">{data.accounts.map((account) => <article key={account.handle}><Cover src={`/api/dashboard/avatar/${account.handle}`} /><div><b>@{account.handle}</b><span>{account.label} · {account.group}</span><small>{fmt(account.total_posts)} posts · {t('threshold')} {fmt(account.hot_threshold)}</small></div><i className={account.is_active ? 'is-active' : ''}>{account.is_active ? t('active') : t('inactive')}</i></article>)}</section> : null}{!loading && tab === 'users' ? <section className="m-settings-list">{data.users.map((user) => <button key={user.email} onClick={() => setEditingUser(user)}><Avatar person={{ ...user, avatarUrl: user.avatar_url }} /><div><b>{user.display_name || displayName(user.email)}</b><span>{user.email}</span><small>{user.operating_role || 'pd'}{user.is_admin ? ' · Admin' : ''}</small></div><ChevronRight size={18} /></button>)}</section> : null}{!loading && tab === 'usage' ? <UsagePanel data={data.usage} /> : null}{!loading && tab === 'notifications' ? <div className="m-command-grid"><Metric label="Slack webhook" value={data.slack?.configured ? 'ON' : 'OFF'} /><Metric label="Groups" value={data.slack?.alert_groups || '—'} /><Metric label="Queue DMs" value="ON" /></div> : null}{!loading && tab === 'system' ? <div className="m-stack"><div className="m-command-grid"><Metric label={t('disk')} value={`${data.disk?.pct_used ?? '—'}%`} hint={`${fmt(data.disk?.free_mb)} MB free`} /><Metric label="OCR remaining" value={fmt(data.ocr?.remaining)} /><Metric label="OCR indexed" value={fmt(data.ocr?.with_text_total)} /></div><div className="m-progress"><span style={{ width: `${Math.min(100, data.disk?.pct_used || 0)}%` }} /></div></div> : null}{!loading && tab === 'reports' ? <ReportsPanel data={data.report} /> : null}{editingUser ? <UserEditSheet user={editingUser} onClose={() => setEditingUser(null)} onSaved={() => { setEditingUser(null); load(); }} /> : null}</div>;
}
