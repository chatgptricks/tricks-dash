...
export function StackActions({ children, onSaved }) {
  ...
  return <Context.Provider value={{ selected, select, selecting, setSelecting, setMenu, propose, separate, findSimilar, drag, suppressClick }}>
    {children}
    {!selecting && createPortal(<button type="button" className="stack-group-launcher" onClick={() => { setError(''); setSelecting(true); }}>{t('Group posts')}</button>, document.body)}
    {selecting && createPortal(<div className="stack-selection-bar" role="toolbar" aria-label={t('Group posts')}><span>{selected.length ? `${selected.length} ${t('selected')}` : t('Select posts to group')}</span><button disabled={selected.length < 2} onClick={() => propose(selected)}>{t('Group selected')}</button>{selected.length ? <button onClick={() => select([])}>{t('Clear')}</button> : null}<button onClick={closeSelection}>{t('Done')}</button></div>, document.body)}
...
export function StackCard({ posts, children }) {
  ...
  return <div className={`stack-card-shell${actions.selecting ? ' is-selecting' : ''}${selected ? ' is-multiselected' : ''}${over ? ' is-drop-target' : ''}`} draggable={actions.selecting}
    onDragStart={(event) => { if (!actions.selecting) { event.preventDefault(); return; } const source = selected ? [...new Set([...actions.selected, ...keys])] : keys; actions.drag.current = source; actions.suppressClick.current = true; event.dataTransfer.effectAllowed = 'move'; event.dataTransfer.setData('application/x-sentient-posts', JSON.stringify(source)); }}
    ...
    onClickCapture={(event) => { if (actions.suppressClick.current) { event.preventDefault(); event.stopPropagation(); return; } if (event.target.closest('.stack-select-toggle')) return; if (actions.selecting || event.shiftKey) { event.preventDefault(); event.stopPropagation(); if (event.shiftKey) actions.setSelecting(true); toggle(); } }}
