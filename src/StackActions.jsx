...
export function StackActions({ children, onSaved, hideLauncher = false }) {
  ...
  return <Context.Provider value={{ selected, select, selecting, setSelecting, setMenu, propose, separate, findSimilar, drag, suppressClick }}>
    {children}
    {!hideLauncher && !selecting && selected.length === 0 ? <div className="stack-group-launcher-host"><button type="button" className="stack-group-launcher" onClick={() => { setError(''); setSelecting(true); }}>{t('Group posts')}</button></div> : null}
