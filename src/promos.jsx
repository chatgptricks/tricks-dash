...
function Card({ item, onSelect }) {
  ...
  return <article className="promo-card" onClick={() => onSelect(item)}>
    ...
    <div className="promo-card-body"><div className="promo-card-top"><Badge value={item.classification} /><span className="promo-review">{item.jev_review ? 'JEV checked' : item.review_status}</span></div>
...
function PromosApp() {
  ...
  const load = useCallback(async () => { if (!user) return; setLoading(true); setError(''); try { const params = new URLSearchParams({ limit: '100' }); if (filters.classification) params.set('classification', filters.classification); if (filters.review) params.set('review', filters.review); const response = await apiFetch(`${API_BASE}/api/admin/promos?${params}`); if (!response.ok) throw new Error(response.status === 403 ? 'Your account does not have Promos access.' : `Unable to load Promos (${response.status}).`); setData(await response.json()); } catch (e) { setError(e.message); } finally { setLoading(false); } }, [user, filters.classification, filters.review]);
