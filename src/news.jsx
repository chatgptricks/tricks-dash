...
function normalizeCachedReview(review) {
  if (!review || review.label !== 'promising') return review;
  const scores = Object.values(review.dimensions || {}).map(value => Number(value?.score) || 0);
  const core = ['insight','audience_value','hook','repurpose','distinctiveness'].map(key => Number(review.dimensions?.[key]?.score) || 0);
  const strong = scores.filter(score => score >= 0.68).length + (Number(review.novelty?.score) >= 0.68 ? 1 : 0);
  const fitsPotential = Number(review.score) >= 0.56 && Number(review.jevSignal) >= 0.40 && Math.min(...core) >= 0.30 && strong >= 2;
  return {...review, label: fitsPotential ? 'potential' : 'not_yet', strongSignalCount: strong};
}
