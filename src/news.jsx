...
function NewsApp() {
  ...
  return <main className="news-shell">
    ...
    <section className="news-hero">
      <div className="news-hero-copy"><span className="news-kicker">SENTIENT NEWS DESK <i /> JEV STORY SCORING</span><h1>Find your next<br /><em>can’t-scroll-past</em> story.</h1><p>One view across 11 feeds. Related coverage is grouped; the best leads rise first. JEV weighs the hook, freshness and evidence before you turn a story into a post.</p><div className="news-hero-proof"><span><b>{healthyFeeds}/{NEWS_FEEDS.length}</b> feeds live</span><span><b>{candidates.length}</b> story leads</span><span><b>{reviewedCount}</b> scored by JEV</span></div></div>
      <div className="news-hero-panel"><div className="news-hero-panel-top"><span className="news-kicker">NEXT BEST ACTION</span><span className="news-live-indicator"><i /> LIVE</span></div><strong className="news-hero-panel-count">{scanning ? `${progress.done}/${progress.total}` : pendingCount}</strong><span className="news-hero-panel-label">{scanning ? 'stories reviewed in this scan' : 'stories ready for JEV review'}</span><button onClick={reviewTop} disabled={!pendingCount || scanning || Boolean(reviewing)}>{scanning ? `Reviewing with JEV · ${progress.done}/${progress.total}` : `Score best ${Math.min(40, pendingCount)} stories`}<span aria-hidden="true">→</span></button><p>Scored stories move into Golden, Potential or Explore later. Stop the scan whenever you like.</p><div className="news-hero-panel-stats"><span><b>{goldenCount}</b> Golden</span><span><b>{potentialCount}</b> Potential</span></div></div>
