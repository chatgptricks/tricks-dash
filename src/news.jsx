...
function StoryCard({ item, review, onReview, busy, locked, saved, onSave, onBrief, failure }) {
  ...
  return <article className={`news-story ${review?.label === 'golden_nugget' ? 'is-golden' : review?.label === 'potential' ? 'is-potential' : ''}`}>
    {item.image && !imageFailed ? <img className="news-story-image" src={item.image} alt="" loading="lazy" onError={() => setImageFailed(true)} /> : <div className="news-story-image news-no-image" aria-hidden="true">✳</div>}
    <div className="news-story-content">
      <div className="news-story-meta"><span className="news-source-pill">{item.sourceType === 'x' ? 'X' : 'NEWS'} · {item.source}</span><span>{item.published ? <><time dateTime={item.published}>{relativeAge(item.published) || dateLabel(item.published)}</time><span className="news-date-detail"> · {dateLabel(item.published)}</span></> : 'Date unavailable'}</span></div>
      <h2>{item.title}</h2>
...
function NewsApp() {
  ...
  return <main className="news-shell">
    ...
    <section className="news-hero">
      <div className="news-hero-copy"><span className="news-kicker">STORY RADAR · POWERED BY JEV</span><h1>Find the stories worth turning into posts.</h1><p>News brings all 11 RSS feeds together, spots related coverage, then scores the hook, freshness and evidence. Viral potential is an editorial signal, not a promise of reach.</p></div>
      <div className="news-stat"><strong>{candidates.length}</strong><span>stories collected</span></div><div className="news-stat news-stat-reviewed"><strong>{reviewedCount}<small> / {candidates.length}</small></strong><span>reviewed by JEV</span></div><div className="news-stat news-stat-leads"><strong>{goldenCount + potentialCount}</strong><span>post-worthy leads</span></div>
      <div className="news-hero-action"><span>{scanning ? `Reviewing ${progress.done} of ${progress.total} stories` : `${pendingCount} stories are waiting for a first look`}</span><button onClick={reviewTop} disabled={!pendingCount || scanning || Boolean(reviewing)}>{scanning ? `JEV ${progress.done}/${progress.total}` : `Review best ${Math.min(40, pendingCount)} stories`}</button><small>JEV assesses them one at a time. You can stop whenever you like.</small></div>
    ...
    <section className="news-source-health" aria-live="polite"><div className="news-health-copy"><span className={`news-health-dot ${healthyFeeds === NEWS_FEEDS.length ? 'is-online' : feedStatuses.length ? 'is-warning' : ''}`} /><div><strong>{loading ? 'Connecting to RSS.app…' : feedStatuses.length ? `${healthyFeeds} of ${NEWS_FEEDS.length} feeds online` : `${NEWS_FEEDS.length} RSS.app feeds configured`}</strong><span>{feedStatuses.length ? `${feedStatuses.reduce((sum,status) => sum + status.count, 0)} items collected · ${candidates.length} unique story clusters` : 'Stories refresh directly from your saved RSS feeds.'}</span></div></div><details><summary>Feed status <span>{feedStatuses.filter(status => status.ok).length || NEWS_FEEDS.length}/{NEWS_FEEDS.length}</span></summary><div className="news-feed-grid">{NEWS_FEEDS.map(feed => { const status = feedStatuses.find(item => item.feed.id === feed.id); return <span key={feed.id} className={status?.ok === false ? 'feed-down' : status?.ok ? 'feed-up' : ''}><i />{feed.label}{status ? ` · ${status.count}` : ''}{status?.error ? ` · ${status.error}` : ''}</span>; })}</div></details></section>
    {error && <div className="news-alert" role="status">{error}</div>}
    <section className="news-toolbar"><div className="news-section-heading"><div><span className="news-kicker">YOUR STORY QUEUE</span><h2>Stories to explore</h2></div><p>Review the strongest leads first, then save or turn a story into a post brief.</p></div><div className="news-filter-tabs" role="tablist" aria-label="Story filters">{filters.map(([key,label,count]) => <button key={key} role="tab" aria-selected={filter === key} className={filter === key ? 'active' : ''} onClick={() => setFilter(key)}>{label}{count !== null && <b>{count}</b>}</button>)}</div><div className="news-search-row"><label className="news-search"><span aria-hidden="true">⌕</span><input aria-label="Search stories" value={query} onChange={event => setQuery(event.target.value)} placeholder="Search stories, sources or topics…" />{query && <button className="news-clear-search" onClick={() => setQuery('')} aria-label="Clear search">×</button>}</label><label className="news-feed-select"><span>Topic</span><select aria-label="Filter stories by topic" value={feedFilter} onChange={event => setFeedFilter(event.target.value)}><option value="all">All topics</option>{feedGroups.map(group => <option value={group} key={group}>{group}</option>)}</select></label><label className="news-sort-select"><span>Sort</span><select aria-label="Sort stories" value={sortBy} onChange={event => setSortBy(event.target.value)}><option value="recommended">Recommended</option><option value="newest">Newest first</option><option value="coverage">Most coverage</option></select></label><span className="news-result-count"><strong>{ranked.length}</strong> shown<span className="news-result-reviewed"> · {reviewedCount} assessed</span></span>{scanning && <button className="news-stop" onClick={() => { stopScan.current = true; }}>Stop review</button>}</div></section>
    ...
    <section className="news-results" aria-label="News stories">{visibleRanked.map(item => <StoryCard key={item.id} item={item} review={reviews[item.id]} busy={reviewing === item.id} locked={scanning || Boolean(reviewing)} failure={failures[item.id]} saved={Boolean(saved[item.id])} onReview={review} onSave={() => setSaved(current => { const next = { ...current }; if (next[item.id]) delete next[item.id]; else next[item.id] = { item }; return next; })} onBrief={() => makeBrief(item)} />)}{!ranked.length && <div className="news-empty"><strong>No stories in this view.</strong><span>Clear the search, change the feed, or review unassessed stories. A story without a review has not been rejected.</span>{!items.length && <button onClick={load} disabled={loading}>{loading ? 'Loading feeds…' : 'Load RSS stories'}</button>}</div>}</section>
