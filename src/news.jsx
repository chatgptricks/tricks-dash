...
function StoryCard({ item, review, onReview, busy, saved, onSave, onBrief, failure }) {
...
function NewsApp() {
  ...
  const [query, setQuery] = useState(''); const [filter, setFilter] = useState('all'); const [saved, setSaved] = useState({}); const [failures, setFailures] = useState({}); const [scanning, setScanning] = useState(false); const [progress, setProgress] = useState({done:0,total:0}); const stopScan = useRef(false); const busyRef = useRef(false); const [brief, setBrief] = useState(null); const [storageReady, setStorageReady] = useState(false);
  useEffect(() => { if (!user?.uid) return; setStorageReady(false); try { const data = JSON.parse(localStorage.getItem(`news-v3:${user.uid}`) || '{}'); setSaved(data.saved || {}); setReviews(Object.fromEntries(Object.entries(data.reviews || {}).filter(([,r]) => Date.now() - r.reviewedAt < 86400000))); } catch {} setStorageReady(true); return () => { stopScan.current = true; }; }, [user?.uid]);
  useEffect(() => { if (!storageReady || !user?.uid) return; try { localStorage.setItem(`news-v3:${user.uid}`, JSON.stringify({saved,reviews})); } catch { setError('Browser storage is full. Saved ideas may not survive a reload.'); } }, [saved,reviews,storageReady,user?.uid]);
  function makeBrief(item) { const r = reviews[item.id]; setBrief({item,text: saved[item.id]?.brief || `WORKING POST BRIEF\n\nTitle: ${item.title}\nAccount: ${r?.targetAccount || 'Choose an account'}\n\nSource evidence:\n${stripHtml(item.description)}\n\nOriginal angle:\n[Explain what changes for the reader, then add a concrete use case.]\n\nSuggested structure:\n1. Hook: what changed and why it matters.\n2. Explain the supported facts.\n3. Show a practical example or comparison.\n4. Give the reader one useful takeaway.\n\nBefore publishing: verify dates, numbers, availability and claims in the source. Add original analysis; do not copy source wording.\n\nSource: ${item.link}`}); }
  ...
  return <main className="news-shell"><ProductHeader current="news" coordinator isDev account={<SettingsMenu email={user.email} avatarUrl={user.photoURL || viewer?.avatar_url} isAdmin={Boolean(viewer?.is_admin)} isDev onSignOut={handleSignOut} />}><h1>News</h1><span className="news-header-subtitle">Find → review → save → create</span><button className="news-refresh" onClick={load} disabled={loading}>Refresh feeds</button><button className="news-review-all" onClick={reviewTop} disabled={!items.length || scanning || Boolean(reviewing)}>Find nuggets · next 25</button></ProductHeader><section className="news-intro"><div><span className="news-kicker">DEV · sourcing lab</span><h1>Find the next post before everyone else.</h1><p>Scans prioritize AI relevance and practical detail; completed reviews rank by JEV editorial value. Save useful ideas and turn them into original post briefs.</p></div><div className="news-metric"><strong>{items.length}</strong><span>candidates loaded</span></div><div className="news-metric gold"><strong>{Object.values(reviews).filter(review => review.label === 'golden_nugget' || review.label === 'gold').length}</strong><span>golden nuggets</span></div></section>{error && <div className="news-alert" role="status">{error}</div>}{feedErrors.length > 0 && <details className="news-feed-errors"><summary>Feed details ({feedErrors.length})</summary><ul>{feedErrors.map(message => <li key={message}>{message}</li>)}</ul></details>}<section className="news-controls"><label>Search stories<input value={query} onChange={e => setQuery(e.target.value)} placeholder="Topic, tool, publisher…" /></label><label>Show<select value={filter} onChange={e => setFilter(e.target.value)}><option value="all">All · best reviewed first</option><option value="golden_nugget">Golden nuggets</option><option value="promising">Promising</option><option value="saved">Saved ideas</option><option value="unreviewed">Not reviewed</option></select></label><span aria-live="polite">{candidates.filter(i => reviews[i.id]).length}/{candidates.length} reviewed · {ranked.length} shown{progress.total > 0 && ` · Scan ${progress.done}/${progress.total}`}</span>{scanning && <button onClick={() => { stopScan.current = true; }}>Stop after current review</button>}</section>
      {brief && <section className="news-brief"><h2>Create an original post</h2><p>Edit the angle and evidence before handing this to production.</p><textarea aria-label="Post brief" value={brief.text} onChange={e => setBrief({...brief,text:e.target.value})} /><div className="news-card-actions"><button onClick={() => { setSaved(current => ({...current,[brief.item.id]:{item:brief.item,brief:brief.text}})); setError('Post brief saved in this browser.'); }}>Save brief</button><button onClick={async () => { try { await navigator.clipboard.writeText(brief.text); setError('Brief copied.'); } catch { setError('Copy failed. Select the brief text and copy it manually.'); } }}>Copy brief</button><button onClick={() => setBrief(null)}>Close</button></div></section>}
      <div className="news-results">{ranked.map(item => <StoryCard key={item.id} item={item} review={reviews[item.id]} busy={scanning || Boolean(reviewing)} onReview={review} failure={failures[item.id]} saved={Boolean(saved[item.id])} onSave={() => setSaved(current => { const next = {...current}; if (next[item.id]) delete next[item.id]; else next[item.id] = {item}; return next; })} onBrief={() => makeBrief(item)} />)}{!ranked.length && <p className="news-empty">No stories in this view. Try All, clear the search, or review more candidates. Unreviewed stories are not rejected stories.</p>}</div>{loading && <div className="news-loading-note">Refreshing RSS feeds…</div>}{!loading && !items.length && <button className="news-load-first" onClick={load}>Load sourcing feeds</button>}</main>;
}
