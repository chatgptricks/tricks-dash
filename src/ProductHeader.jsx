...
export default function ProductHeader({ current, coordinator = false, account, children, count = 0 }) {
  ...
  return <header className="product-header">
    ...
    <nav className="product-nav" aria-label="Sentient tools">
      {productSections.filter((item) => !item.restricted || coordinator).map((item) => <a key={item.id} href={sectionHref(item)} aria-current={current === item.id ? 'page' : undefined}>{language === 'es' ? item.es : item.label}{item.id === 'queue' && count > 0 ? <b>{count > 99 ? '99+' : count}</b> : null}</a>)}
    </nav>
