...
export function planQueueDrop({ tasks, target, designerEmail, scheduledDate, desiredStart, notBefore = QUEUE_DAY_START }) {
  ...
  const targetStart = findSlot(target, desiredStart, fixed, notBefore);
  if (targetStart === null) return { ok: false, error: 'There is no available space for this request.' };
  ...
  for (const task of pending) {
    const start = findSlot(task, task.scheduledStartMinutes ?? QUEUE_DAY_START, placed, notBefore);
