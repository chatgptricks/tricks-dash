...
function QueueApp({ user }) {
  ...
  return <main className="queue-page scheduler-page">
    ...
    <Detail task={open} tags={data?.tags || []} canCoordinate={coordinator} canDuplicate={Boolean(coordinator || (canSelfAssign && open?.designerEmail === data?.viewer?.email))} isOwner={open?.designerEmail === data?.viewer.email || data?.viewer.isAdmin} isTrainee={Boolean(data?.viewer?.operatingRoles?.includes('trainee') && !data?.viewer?.isAdmin)} pendingTickets={openPendingTickets} onReviewTicket={reviewTicket} notice={detailNotice} history={history} historyLoading={historyLoading} onClose={closeDetail} onAction={action} onCancel={cancel} onEdit={edit} onNotify={resend} onUpload={upload} onDownload={download} onRequestPP={requestPP} onRequestCancellation={requestCancellation} onRequestMove={requestMove} onRequestTraineeReview={requestTraineeReview} onDuplicate={(task) => duplicateRequest(task.id)} timeZone={simulatedTimeZone} />
