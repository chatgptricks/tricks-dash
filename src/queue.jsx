...
Object.assign(COPY.en, {
  ...
  pick: 'Pick', pickTitle: 'Pick a request', pickHelp: 'Choose a request from the production pool.', nextRequest: 'Next', assignRequest: 'Assign', noPickRequests: 'There are no requests available in the pool.', pickedRequest: 'Request assigned to your schedule.', pickPriority: 'Priority',
...
Object.assign(COPY.es, {
  ...
  pick: 'Pick', pickTitle: 'Elegir un request', pickHelp: 'Elige un request del pool de producción.', nextRequest: 'Siguiente', assignRequest: 'Asignar', noPickRequests: 'No hay requests disponibles en el pool.', pickedRequest: 'Request asignado a tu agenda.', pickPriority: 'Prioridad',
...
function PickModal({ requests, busy, onClose, onAssign }) {
  const { t } = useQueuePreferences();
...
function QueueApp({ user }) {
  ...
  const pickPool = useMemo(() => (data?.pickRequests || []).filter((task) => task.status === 'pool'), [data]);
  ...
  return <main className="queue-page scheduler-page">
    <header className="queue-topbar">
      ...
      <div className="queue-actions">
        ...
        {!coordinator && pickPool.length ? <button type="button" className={`queue-pick-button${pickOpen ? ' is-active' : ''}`} onClick={() => setPickOpen(true)}><Check size={14} />{t('pick')}</button> : null}
    ...
    {pickOpen ? <PickModal requests={pickPool} busy={pickBusy} onClose={() => setPickOpen(false)} onAssign={pickRequest} /> : null}
