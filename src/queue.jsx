...
const COPY = {
  en: {
    ...
    priority: 'Priority', priorityLow: 'Low', priorityMedium: 'Medium', priorityHigh: 'High', priorityUrgent: 'Urgent', priorityMix: 'Priority mix', highPriority: 'High priority',
  },
  es: {
    ...
    priority: 'Prioridad', priorityLow: 'Baja', priorityMedium: 'Media', priorityHigh: 'Alta', priorityUrgent: 'Urgente', priorityMix: 'Niveles de prioridad', highPriority: 'Prioridad alta',
  },
};
...
function Scheduler({ data, draft, setDraft, selectedDate, designerScope, onOpen, onError }) {
  ...
  const allTasks = useMemo(() => {
    const byId = new Map();
    [...(data.planningRequests || []), ...data.requests, ...draft].forEach((task) => byId.set(task.id, task));
    return [...byId.values()];
  }, [data.planningRequests, data.requests, draft]);
