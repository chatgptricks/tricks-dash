...
import { ArrowLeft, BarChart3, CalendarDays, ChevronLeft, ChevronRight, LoaderCircle, LogOut, Send, Settings, X } from 'lucide-react';
...
function AdminTools({ report, loading, error, onClose }) {
  ...
  return <section className="queue-admin-tools"><header><div><p className="scheduler-eyebrow">Admin workspace</p><h2>Production reports</h2></div><button type="button" onClick={onClose} aria-label="Close admin tools"><X size={16} /></button></header>{loading ? <div className="queue-admin-loading"><LoaderCircle className="queue-spin" />Loading report…</div> : null}{error ? <p className="queue-admin-error">{error}</p> : null}{report ? <><div className="queue-admin-metrics">{metric('pool', 'In pool')}{metric('scheduled', 'Scheduled')}{metric('in_progress', 'In progress')}{metric('completed', 'Ready to close')}{metric('closed', 'Closed')}</div><div className="queue-admin-designers"><div><h3>Designer workload</h3><p>Active requests and planned production points.</p></div><div className="queue-admin-designer-list">{report.designers.map((designer) => <span key={designer.email}><b>{designer.email.split('@')[0]}</b><small>{designer.activeRequests} active · {designer.productionPoints} PP</small></span>)}</div></div><a className="queue-admin-settings" href={`${import.meta.env.BASE_URL}?view=admin`} target="_blank" rel="noreferrer"><Settings size={14} />Open dashboard settings</a></> : null}</section>;
}
