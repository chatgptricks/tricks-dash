...
function Scheduler({ data, draft, setDraft, selectedDate, onOpen, onDragStart }) {
  ...
  const drop = (event, designer) => { event.preventDefault(); if (!coordinator) return; const id = Number(event.dataTransfer.getData('queue-task')); const source = data.requests.find((task) => task.id === id) || draft.find((task) => task.id === id); if (!source || source.status === 'in_progress') return; const rect = event.currentTarget.getBoundingClientRect(); const pointerStart = Math.round(((((event.clientX - rect.left) / rect.width) * 720) + viewStart) / 10) * 10; const start = Math.max(START, today ? earliestStart : START, pointerStart); if (start + source.durationMinutes > 1440) return; setDraft((current) => [...current.filter((item) => item.id !== id), { ...source, designerEmail: designer, scheduledDate: selectedDate, scheduledStartMinutes: start, status: 'scheduled' }]); };
