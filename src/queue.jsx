...
const COPY = {
  en: {
    ...
    priority: 'Priority', priorityLow: 'Low', priorityMedium: 'Medium', priorityHigh: 'High', priorityUrgent: 'Urgent', priorityMix: 'Priority mix', highPriority: 'High priority', spaceError: 'There is no available space for this request.',
  },
  es: {
    ...
    priority: 'Prioridad', priorityLow: 'Baja', priorityMedium: 'Media', priorityHigh: 'Alta', priorityUrgent: 'Urgente', priorityMix: 'Niveles de prioridad', highPriority: 'Prioridad alta', spaceError: 'No hay espacio disponible para este request.',
  },
};
...
function Scheduler({ data, draft, setDraft, selectedDate, designerScope, onOpen, onError }) {
  ...
  const drop = (event, designer) => {
    ...
    if (!result.ok) { onError(result.error || t('spaceError')); return; }
    setDraft((current) => {
