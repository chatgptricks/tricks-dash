...
function Detail({ task, tags, canCoordinate, isOwner, isTrainee, pendingTickets = [], onReviewTicket, notice, history, historyLoading, onClose, onAction, onCancel, onEdit, onNotify, onUpload, onDownload, onRequestPP, onRequestCancellation, onRequestMove, onRequestTraineeReview, timeZone = QUEUE_TIME_ZONE }) {
  const { t } = useQueuePreferences();
  ...
  useEffect(() => { setLink(task?.finalPermalink || ''); setReason(''); setEditing(false); setRequestMode(''); setRequestedPP(task?.productionPoints || 1); setMoveDate(scheduleDateForViewer(task?.scheduledDate, task?.scheduledStartMinutes ?? QUEUE_DAY_START, timeZone)); setMoveStart(scheduleTimeForViewer(task?.scheduledDate, task?.scheduledStartMinutes ?? QUEUE_DAY_START, timeZone)); setRequestReason(''); setForm({ productionPoints: task?.productionPoints || 1, priority: task?.priority || 'medium', tags: task?.tags || [], brief: task?.brief || '', notes: task?.notes || '', references: task?.references?.join('\n') || '' }); }, [task?.id, timeZone]);
  ...
  const designerRequestActions = canRequestChange ? <div className="queue-designer-ticket-actions">{!requestMode ? <><button type="button" disabled={busy || !['scheduled', 'in_progress'].includes(task.status)} onClick={() => { setRequestedPP(task.productionPoints); setRequestMode('pp'); }}><TimerReset size={13} />{t('requestPPChange')}</button><button type="button" disabled={busy || task.status !== 'scheduled'} onClick={() => { setMoveDate(scheduleDateForViewer(task.scheduledDate, task.scheduledStartMinutes ?? QUEUE_DAY_START, timeZone)); setMoveStart(scheduleTimeForViewer(task.scheduledDate, task.scheduledStartMinutes ?? QUEUE_DAY_START, timeZone)); setRequestMode('move'); }}><Clock3 size={13} />{t('requestMove')}</button><button type="button" disabled={busy} onClick={() => setRequestMode('cancel')}><Ban size={13} />{t('requestCancellation')}</button></> : <div className="queue-designer-ticket-form">{requestMode === 'pp' ? <label>{t('requestedPP')}<input type="number" min="1" value={requestedPP} onChange={(event) => setRequestedPP(event.target.value)} /></label> : requestMode === 'move' ? <><strong>{t('moveRequest')}</strong><small>{t('moveHelp')}</small><label>{t('moveTo')}<input type="date" value={moveDate} onChange={(event) => setMoveDate(event.target.value)} /><input type="time" step="600" value={moveStart} onChange={(event) => setMoveStart(event.target.value)} /></label></> : <strong>{t('requestCancellation')}</strong>}<label>{t('requestReason')}<textarea value={requestReason} onChange={(event) => setRequestReason(event.target.value)} /></label><div><button type="button" className="is-send" disabled={busy || (requestMode === 'pp' && (!requestedPP || Number(requestedPP) === Number(task.productionPoints))) || (requestMode === 'move' && (!moveDate || !moveStart || (moveDate === scheduleDateForViewer(task.scheduledDate, task.scheduledStartMinutes ?? QUEUE_DAY_START, timeZone) && minutesFromTime(moveStart) === queueScheduleClock(task.scheduledDate, task.scheduledStartMinutes ?? QUEUE_DAY_START, timeZone).minutes)))} onClick={sendDesignerRequest}>{busy ? <LoaderCircle className="queue-spin" size={13} /> : <Send size={13} />}{t('sendRequest')}</button><button type="button" disabled={busy} onClick={() => { setRequestMode(''); setRequestReason(''); }}>{t('cancel')}</button></div></div>}</div> : null;
