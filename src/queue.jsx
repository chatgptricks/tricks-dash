...
function Scheduler({ data, draft, setDraft, onDraftChange, selectedDate, designerScope, onOpen, onError }) {
  ...
  return <section className="scheduler" ref={scrollRef}><button type="button" className="scheduler-center-now" onClick={centerNow} title={t('centerNow')} aria-label={t('centerNow')}><LocateFixed size={15} /><span>{t('centerNow')}</span></button><div className="scheduler-canvas"><div className="scheduler-time-head"><span>{t('designer')}</span><div>{Array.from({ length: 24 }, (_, hour) => <b key={hour} style={{ left: `${hour * (100 / 24)}%` }}>{time(hour * 60)}</b>)}</div></div>{visibleDesigners.map((designer) => { const queueEligible = designer.isQueueDesigner !== false; const initials = designer.email.split('@')[0].slice(0, 2).toUpperCase(); const role = schedulerUserRole(designer, t); const accounts = designer.accounts?.map((account) => `@${account}`).join(' · ') || t('noAccounts'); return <div className={`scheduler-row${queueEligible ? '' : ' is-non-queue-user'}`} key={designer.email}><header><div className="scheduler-user-identity"><span className="scheduler-user-avatar"><span aria-hidden="true">{initials}</span>{designer.avatarUrl ? <img src={designer.avatarUrl} alt="" loading="lazy" onError={(event) => { event.currentTarget.hidden = true; }} /> : null}</span><span className="scheduler-user-copy"><b>{designer.email.split('@')[0]}</b><small>{[role, accounts].filter(Boolean).join(' · ')}</small></span></div></header><div className="scheduler-track" onDragOver={(event) => previewDrop(event, designer.email)} onDragLeave={(event) => { if (!event.currentTarget.contains(event.relatedTarget)) setDropPreview(null); }} onDrop={(event) => drop(event, designer.email)}>{Array.from({ length: 25 }, (_, hour) => <i key={hour} style={{ left: `${hour * (100 / 24)}%` }} />)}{dropPreview?.designer === designer.email ? <span className={`scheduler-drop-preview${previewNextDay ? ' is-next-day' : ''}`} style={{ left: `${previewLeft}%`, width: `${previewWidth}%` }}><b>@{dropPreview.target.post.account}</b><small>{previewNextDay ? `${displayDate(dropPreview.target.scheduledDate, language)} · ` : ''}{time(dropPreview.target.scheduledStartMinutes)} · {dropPreview.target.durationMinutes} min</small></span> : null}{merged(designer.email).map((task) => <TaskBlock key={task.id} task={task} editable={coordinator && (!task.isDraft || task.draftCoordinatorEmail === data.viewer.email)} onOpen={onOpen} />)}</div></div>; })}{today ? <div className="scheduler-day-overlay"><span className="scheduler-now-global" style={{ left: `${nowPosition}%` }}><b>{t('now')}</b></span></div> : null}</div></section>;
}
